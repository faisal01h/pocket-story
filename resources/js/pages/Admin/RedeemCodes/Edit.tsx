...
export default function Edit({ redeemCode, providers }: { redeemCode: any, providers: any[] }) {
    const breadcrumbs: BreadcrumbItem[] = [
        { title: 'Dashboard', href: '/dashboard' },
        { title: 'Redeem Codes', href: '/admin/redeem-codes' },
        { title: 'Edit', href: `/admin/redeem-codes/${redeemCode.id}/edit` },
    ];

    const { data, setData, put, processing, errors } = useForm({
        code: redeemCode.code,
        llm_model_id: (redeemCode.llm_model_id?.toString() || null) as string | null,
        period: redeemCode.period,
        max_tokens: redeemCode.max_tokens,
        usage_limit: redeemCode.usage_limit,
        expires_at: redeemCode.expires_at ? new Date(redeemCode.expires_at).toISOString().slice(0, 16) : '',
        is_active: !!redeemCode.is_active,
    });

    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        put(`/admin/redeem-codes/${redeemCode.id}`);
    };

    return (
        <AuthenticatedLayout breadcrumbs={breadcrumbs}>
            <Head title="Edit Redeem Code" />
            ...
            <div className="py-6">
                <div className="mx-auto max-w-2xl px-4 sm:px-6 lg:px-8">
                    ...
                    <Card className='py-5'>
                        <CardHeader>
                            <CardTitle>Edit Redeem Code</CardTitle>
                        </CardHeader>
                        <CardContent>
                            <form onSubmit={handleSubmit} className="space-y-6">
                                ...
                                <div className="pt-4">
                                    <Button type="submit" className="w-full" disabled={processing}>
                                        {processing ? 'Updating...' : 'Update Redeem Code'}
                                    </Button>
                                </div>
                            </form>
                        </CardContent>
